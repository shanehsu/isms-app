...
var NavigationComponent = (function () {
    ...
    NavigationComponent.prototype.ngOnInit = function () {
        // 一開始假設 尚未登入
        this.user = undefined;
        this.navigationItems = this.config.navigationItems.filter(function (item) { return item.group.includes('guests'); });
        this.loading = false;
        // 獲取登入狀況
    ...
    NavigationComponent.prototype.checkLoginState = function () {
        ...
        if (this.authService.has_token) {
            ...
            this.authService.me().then(function (user) {
                _this.user = user;
                _this.loading = false;
                _this.navigationItems = _this.config.navigationItems.filter(function (item) { return item.group.includes(user.group); });
    ...
    NavigationComponent.prototype.login = function (email) {
        ...
        this.authService.login(email).then(function () {
            _this.authService.me().then(function (user) {
                ...
                _this.navigationItems = _this.config.navigationItems.filter(function (item) { return item.group.includes('guests'); });
                console.error(err);
